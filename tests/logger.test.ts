...
import { TinyITLogger, createLogger } from '../src/logger.js';
import { TinyITClient } from '../src/client.js';

// Mock the client
jest.mock('../src/client.js');
const MockedTinyITClient = TinyITClient as jest.MockedClass<typeof TinyITClient>;

describe('TinyITLogger', () => {
  let mockClient: jest.Mocked<TinyITClient>;
  let logger: TinyITLogger;
  ...
  beforeEach(() => {
    ...
    mockClient = {
      ...
      forceProcessQueue: jest.fn().mockResolvedValue(undefined),
      clearQueue: jest.fn(),
    } as any;
  ...
  describe('Input Validation', () => {
    it('should reject empty message', async () => {
      await expect(logger.info('')).rejects.toThrow('Log message is required and must be a string');
      await expect(logger.error('')).rejects.toThrow('Log message is required and must be a string');
      await expect(logger.warn('')).rejects.toThrow('Log message is required and must be a string');
    });

    it('should reject non-string message', async () => {
      await expect(logger.info(null as any)).rejects.toThrow('Log message is required and must be a string');
      await expect(logger.info(123 as any)).rejects.toThrow('Log message is required and must be a string');
      await expect(logger.info({} as any)).rejects.toThrow('Log message is required and must be a string');
    });

    it('should reject invalid metadata', async () => {
      await expect(logger.info('Test', null as any)).rejects.toThrow('Log metadata must be an object');
      await expect(logger.info('Test', 'string' as any)).rejects.toThrow('Log metadata must be an object');
    });

    it('should accept valid metadata types', async () => {
      await expect(logger.info('Test', { key: 'value' })).resolves.not.toThrow();
      await expect(logger.info('Test', { number: 123 })).resolves.not.toThrow();
      await expect(logger.info('Test', { nested: { object: true } })).resolves.not.toThrow();
      await expect(logger.info('Test', { array: [1, 2, 3] })).resolves.not.toThrow();
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle client send errors with context', async () => {
      ...
      try {
        ...
        expect(error.message).toContain('Failed to send error log: Network error');
    ...
    it('should handle non-Error objects', async () => {
      ...
      try {
        ...
        expect(error.message).toContain('Failed to send info log: Unknown error');
      }
    });
  });
  ...
  describe('Concurrent Logging', () => {
    it('should handle multiple concurrent log calls', async () => {
      ...
      expect(mockClient.send).toHaveBeenNthCalledWith(1, '/logs', expect.objectContaining({ level: 'info', message: 'Message 1' }));
      expect(mockClient.send).toHaveBeenNthCalledWith(2, '/logs', expect.objectContaining({ level: 'error', message: 'Message 2' }));
      expect(mockClient.send).toHaveBeenNthCalledWith(3, '/logs', expect.objectContaining({ level: 'warn', message: 'Message 3' }));
      expect(mockClient.send).toHaveBeenNthCalledWith(4, '/logs', expect.objectContaining({ level: 'info', message: 'Message 4' }));
    });
  });
});
...
describe('createLogger helper function', () => {
  ...
  it('should create logger with project secret', () => {
    const logger = createLogger('https://api.test.com', 'test-key', 'test-secret');
